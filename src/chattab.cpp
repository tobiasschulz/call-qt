...
ChatTab::ChatTab(const Contact& contact, QWidget *parent)
		: Tab("Chat", QIcon(), parent), ui(new Ui::ChatTab), m_contact(contact), m_chatclient(contact),
			m_thread("ChatThread")
{
	ui->setupUi(this);
	m_thread.start();
	...
	QObject::connect(ContactList::instance(), &ContactList::hostOnline, this, &ChatTab::onHostOnline);
	QObject::connect(ContactList::instance(), &ContactList::hostOffline, this, &ChatTab::onHostOffline);
